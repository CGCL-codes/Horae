...
/******************* static variables *************************/
static GssTimeslice* baseline_gss;
/******************* static variables *************************/
...
// gss-timeslice insert functions
int baselineInsert(VAR var, string filename);
// para query functions
int64_t edgeFrequenceBaseline(GssTimeslice& GssTimeslice, int64_t s, int64_t d, int64_t start, int64_t end);
...
int64_t nodeFrequenceBaseline(GssTimeslice& GssTimeslice, int64_t v, int type, int64_t start, int64_t end);
...
#endif
	baseline_gss = new GssTimeslice(var.gl, var.width, var.depth, var.fingerprintLen, var.row_addrs, var.col_addrs);
...
int64_t nodeFrequenceBaseline(GssTimeslice& GssTimeslice, int64_t v, int type, int64_t start, int64_t end) {
	...
	for (int i = start; i <= end; i++) {
		...
		result += GssTimeslice.nodeQuery(vertex, type);
	}
	return result;
}
int nodeFrequenceBaselineTest_single(string input_dir, string output_dir, string dataset_name, int num, int query_times, bool write, int flag, int line) {
