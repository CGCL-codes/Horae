#ifndef _GssTimeslice_H
#define _GssTimeslice_H
#include <iostream>
#include <string>
#include <vector>
#include <queue>
...
public:
	GssTimeslice(uint32_t granularity, uint32_t width, uint32_t depth, uint32_t fingerprintLength, uint32_t row_addrs = 4, uint32_t column_addrs = 4);
	~GssTimeslice();
...
GssTimeslice::GssTimeslice(uint32_t granularity, uint32_t width, uint32_t depth, uint32_t fingerprintLength, uint32_t row_addrs, uint32_t column_addrs):
granularity(granularity), width(width), depth(depth), fingerprintLength(fingerprintLength), row_addrs(row_addrs), column_addrs(column_addrs) {
	cout << "GssTimeslice::GssTimeslice(granularity: " << granularity 
...
int GssTimeslice::getMinIndex(uint32_t* a, int length) {
	uint32_t min = a[0];
	int index = 0;
...
uint32_t GssTimeslice::getGranularity() const{
	if(this == NULL) {
...
void GssTimeslice::bucketCounting() {
	...
	cout << "GssTimeslice room_count = " << room_count << ", total room = " << (width * depth * SLOTNUM) << ", space usage is " << 
			(double)room_count / (double)(width * depth * SLOTNUM) * 100 << "%" << endl;
	cout << "GssTimeslice bucket_count = " << bucket_count << ", total bucket = " << (width * depth) << ", space usage is " << 
...
void GssTimeslice::insert(string src, string dst, weight_type weight) {
...
weight_type GssTimeslice::edgeQuery(string src, string dst) {
	uint32_t hash_src = (*hfunc[HASH])((unsigned char*)(src.c_str()), src.length());
	uint32_t hash_dst = (*hfunc[HASH])((unsigned char*)(dst.c_str()), dst.length());
	uint32_t mask = (1 << fingerprintLength) - 1;
	uint16_t fp_src = hash_src & mask;
...
weight_type GssTimeslice::nodeQuery(string vertex, int type) { // vertex is the ID of the queried node, function for node query.
