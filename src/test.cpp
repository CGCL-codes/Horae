...
int insert_horae_parallel(Horae* horae, unsigned int fpLength, int level, string filename, int line);
...
int main(int argc, char* argv[]) {
	...
	string back_addr = "_20200906_HORAE";
    ...
    HORAE_VAR horae_var = { startTime, 86400, 1, 740, 739, 6 };

	string test_situation_dir = "lkml_gl_86400_740_739_6" + back_addr + +"//";
    ...
    //edgeFrequencePgssTest(para_query, horae_parallel, input_dir, output_dir, dataset_name, num, query_times, writeflag);
...
int baselineInsert(HORAE_VAR var, string filename) {
	...
	while (!ifs.eof()) {
		...
		string sv = to_string(s) + "+" + to_string((int)ceil((double)tt / (double)baseline_layer->getGranularity()));
		string dv = to_string(d) + "+" + to_string((int)ceil((double)tt / (double)baseline_layer->getGranularity()));
...
int insert_horae_parallel(Horae *horae, unsigned int fpLength, int level, string filename, int line) {
	...
	while (!ifs.eof()) {
		...
		int tt = ceil((double)(t - horae->getStartTime()) / (double)horae->getTimesliceLength());
		
		if ((flag == 1) && (tt > horae->getLayer(level)->getGranularity())) {
			flag = 0;
			int line = datanum + 1;
			
			Layer* layer = new Layer(*(horae->getLayer(level)));
			//Layer *layer = new Layer((1<<(level+1)), 740, 739, 6);
			printf("level = %d, layer = %p\n", level + 1, layer);
			if(layer == NULL) {
				cout << "new failed!" << endl;
				getchar();
			}
			//m.lock();
			horae->addLayer(layer);
			horae->levelInsert(level + 1, s, d, w, t);
			//m.unlock();	

			unsigned int length = ifs.tellg();
			//cout << "(" << s << ", " << d << ", " << w << ", " << t << ") -- " << " level = " << level << endl;
			child = new thread(insert_horae_parallel, horae, length, level + 1, filename, line);
			if(child == NULL) {
				cout << "thread failed!" << endl;
				getchar();
			}
		}
		horae->levelInsert(level, s, d, w, t);
...
int horaeParallelInsert(HORAE_VAR var, string filename) {
	...
	while (!ifs.eof()) {
		...
		if ((flag == 1) && (tt > (horae_parallel->getLayer(level)->getGranularity()))) {
			...
			unsigned int length = ifs.tellg();
			//cout << "(" << s << ", " << d << ", " << w << ", " << t << ") -- " << " level = " << level << endl;
			Layer* layer = new Layer(*(horae_parallel->getLayer(level)));
			//Layer *layer = new Layer((var.gl << (level+1)), var.width, var.depth, var.fingerprintLength);
			printf("level = %d, layer = %p\n", level + 1, layer);
			if(layer == NULL) {
				cout << "new failed!" << endl;
				getchar();
