...
int baselineInsert(HORAE_VAR var, string filename) {
...
#endif
	baseline_gss = new Layer(var.gl, var.width, var.depth, var.fingerprintLen);
	...
	gettimeofday( &bpoint_start, NULL);
		...
		double total = 0;
		string::size_type idx = filename.find("stackoverflow");
		if(idx != string::npos) {
			total = 63497050;
		}
		else {
			idx = filename.find("caida");
			if(idx != string::npos) {
				total = 403436907;
			}
			else {
				idx = filename.find("lkml");
				if(idx != string::npos) {
					total = 1096440;
				}
			}
...
int pgssSequentialInsert(HORAE_VAR var, string filename) {
	...
	cout << "Inserting..." << endl;
#endif
	pgss_sequential = new Horae(var.startTime, var.granularityLength, var.gl, var.width, var.depth, var.fingerprintLen);	
	int64_t s, d;
	weight_type w;
	time_type t;
...
#endif
			Layer* gs = new Layer(*(pg->getLayer(level)));
#if defined(DEBUG) || defined(TINSTIME)
...
int pgssParallelInsert(HORAE_VAR var, string filename) {
	...
	cout << "Inserting..." << endl;
#endif
	pgss_parallel = new Horae(var.startTime, var.granularityLength, var.gl, var.width, var.depth, var.fingerprintLen);
	int64_t s, d;
	weight_type w;
	time_type t;
	int flag = 1, level = 0, datanum = 0;
	...
	while (!ifs.eof()) {
		...
		if ((flag == 1) && (tt > (pgss_parallel->getLayer(level)->getGranularity()))) {
			...
			gettimeofday( &matrix_s1, NULL);
#endif
			Layer* gs = new Layer(*(pgss_parallel->getLayer(level)));
#if defined(DEBUG) || defined(TINSTIME)
			gettimeofday( &matrix_e1, NULL);
			double matrix_time1 = (matrix_e1.tv_sec - matrix_s1.tv_sec) +  (matrix_e1.tv_usec - matrix_s1.tv_usec) / 1000000.0;
			cout << "Level " << (level + 1) << ", Line = " << line << ", Matrix Time = " << matrix_time1 << " s" << endl;
